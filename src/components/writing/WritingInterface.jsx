...
const WritingInterface = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // Check if both projectId and chapterId are available
        if (!projectId || !chapterId) {
          setAlert('Missing project or chapter information', 'error');
          return;
        }
      ...
      <AppBar position="fixed" color="default" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}>
        <Toolbar>
          ...
          <Typography variant="h6" noWrap sx={{ flexGrow: 1, ml: 2 }}>
            {currentProject?.title || 'No Project Selected'} - {currentChapter?.title || 'No Chapter Selected'}
          </Typography>
          
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Typography variant="body2" sx={{ mr: 3 }}>
              {wordCount} / {currentProject?.wordCountGoal || 0} words
            </Typography>
          ...
          {loading ? (
            ...
            <Box sx={{ textAlign: 'center', py: 4 }}>
              ...
              <Typography variant="body1" color="text.secondary">
                Please select a project and chapter to start writing
              </Typography>
              <Button
                variant="contained"
